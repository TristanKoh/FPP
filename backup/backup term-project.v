...
Theorem concatenation_of_two_list_bcis_with_n' :
  forall (bcis1 bcis2 : list byte_code_instruction)
    ...
    /\
    (verify_aux bcis1 n = None ->
     verify_aux (bcis1 ++ bcis2) n = None).
Proof.
  intro bcis1.
  induction bcis1 as [ | bci bcis1 IHbcis]; intros bcis2 n; split.
  ...
  - intros n' H_verify_aux.
    ...
    case bci as [ n'' | | ].
    + assert (IHbcis := IHbcis (S n)).
      destruct IHbcis as [IHbcis_Some _].
      rewrite -> (IHbcis_Some n' H_verify_aux).
      reflexivity.
    + case n as [ | n''].
  ...
  - intros H_verify_aux.
    ...
    case bci as [ n'' | | ].
    + assert (IHbcis := IHbcis (S n)).
      destruct IHbcis as [_ IHbcis_None].
      rewrite -> (IHbcis_None H_verify_aux).
      reflexivity.
    + case n as [ | n''].
      * reflexivity.
      * case n'' as [ | n'''].
...
Proof.
  ...
  induction ae as [n' | ae1 IHae1 ae2 IHae2 | ae1 IHae1 ae2 IHae2].
  - rewrite -> (fold_unfold_compile_aux_Literal n').
    rewrite -> 2 (fold_unfold_verify_aux_cons (PUSH n') nil).
    rewrite -> 2 fold_unfold_verify_aux_nil.
    intros _.
    reflexivity.
  - rewrite -> (fold_unfold_compile_aux_Plus ae1 ae2).
    assert (H_concat1 := concatenation_of_two_list_bcis_with_n (compile_aux ae1) (compile_aux ae2 ++ ADD :: nil)).
    rewrite -> 2 H_concat1.
    assert (H_concat2 := concatenation_of_two_list_bcis_with_n (compile_aux ae2) (ADD :: nil)).
    case (compile_aux ae1) as [ | bci bcis] eqn:H_ae1.
    + case (compile_aux ae2) as [ | bci bcis] eqn:H_ae2.
      * rewrite -> 2 fold_unfold_verify_aux_nil.
        admit.
      * rewrite -> 2 fold_unfold_verify_aux_nil.
        rewrite -> 2 H_concat2.
        rewrite -> (fold_unfold_verify_aux_cons bci bcis 0).
...
Proof.
  ...
  - rewrite -> (fold_unfold_compile_aux_Plus ae1 ae2).
    ...
    assert (H_concat := concatenation_of_two_list_bcis_with_n' (compile_aux ae1) (compile_aux ae2 ++ ADD :: nil) 0).
    destruct H_concat as [H_verify_aux_Some1 _].
    ...
    assert (H_verify_aux_Some1 := H_verify_aux_Some1 1 H_verify_aux1).
    rewrite -> H_verify_aux_Some1.
    
    assert (H_concat := concatenation_of_two_list_bcis_with_n' (compile_aux ae2) (ADD :: nil) 1).
    destruct H_concat as [H_verify_aux_Some2 _].
    assert (H_verify_aux_Some2 := H_verify_aux_Some2 2 H_verify_aux2).
    rewrite -> H_verify_aux_Some2.
  ...
  - rewrite -> (fold_unfold_compile_aux_Minus ae1 ae2).
    unfold verify.
    unfold verify in IHae1, IHae2.
    assert (H_concat := concatenation_of_two_list_bcis_with_n' (compile_aux ae1) (compile_aux ae2 ++ SUB :: nil) 0).
    destruct H_concat as [H_verify_aux_Some1 _].
    assert (H_verify_aux1 := verify_aux_true_implies_Some ae1 0 IHae1).
    assert (H_verify_aux2 := verify_aux_true_implies_Some ae2 1 IHae2).
    assert (H_verify_aux_Some1 := H_verify_aux_Some1 1 H_verify_aux1).
    ...
    assert (H_concat := concatenation_of_two_list_bcis_with_n' (compile_aux ae2) (SUB :: nil) 1).
    destruct H_concat as [H_verify_aux_Some2 _].
    assert (H_verify_aux_Some2 := H_verify_aux_Some2 2 H_verify_aux2).
    rewrite -> H_verify_aux_Some2.
